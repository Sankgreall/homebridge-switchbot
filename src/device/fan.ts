  ...
  async pushRotationSpeedChanges(): Promise<void> {
    ...
    if (this.Fan.SwingMode !== this.accessory.context.SwingMode) {
      ...
      try {
        ...
        await this.apiError(e)
        this.errorLog(`failed pushRotationSpeedChanges with ${this.device.connectionType} Connection, Error Message: ${e.message || String(e)}`)
      }
    } else {
  ...
  async pushSwingModeChanges(): Promise<void> {
    ...
    if (this.Fan.SwingMode !== this.accessory.context.SwingMode) {
      ...
      try {
        ...
        await this.apiError(e)
        this.errorLog(`failed pushSwingModeChanges with ${this.device.connectionType} Connection, Error Message: ${e.message || String(e)}`)
      }
    } else {
